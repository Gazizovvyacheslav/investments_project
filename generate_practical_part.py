...
def render_report(
    ...
    def calrow(row: pd.Series) -> str:
        return (
            f"| {row['strategy_type']} | {int(row['window'])} | {row['rebalance_freq']} | "
            f"{float(row['risk_cap_annual']):.0%} | {row['val_sharpe']} | {row['val_ES95']} | "
            f"{row['val_MDD']} | {row['gate_passed']} |"
        )

    calibration_rows = "\n".join(calrow(row) for _, row in top_cal.iterrows())

    ns = metrics_test.loc["Opt No-Short"]
    ew = metrics_test.loc["Equal Weight"]
    spy = metrics_test.loc["SPY"]
    short = metrics_test.loc["Opt Short"]
...
На тестовом интервале no-short стратегия показала cumulative return {pct(ns['cumulative_return'])} и annualized return {pct(ns['annualized_return'])}. По абсолютной доходности она уступила Equal Weight ({pct(ew['annualized_return'])}) и SPY ({pct(spy['annualized_return'])}). Это означает, что в рассматриваемом периоде простые рыночные бенчмарки оказались сильнее по сырой доходности.

По Sharpe ratio `Opt No-Short` также оказался ниже обоих бенчмарков: {ns['sharpe_ratio']:.3f} против {ew['sharpe_ratio']:.3f} у Equal Weight и {spy['sharpe_ratio']:.3f} у SPY. При этом стратегия сохранила более низкую рыночную чувствительность: beta к SPY составила {ns['beta_to_spy']:.3f}. Максимальная просадка no-short портфеля равна {pct(ns['max_drawdown'])}, что близко к SPY ({pct(spy['max_drawdown'])}) и немного хуже Equal Weight ({pct(ew['max_drawdown'])}). По ES95 no-short портфель также уступил бенчмаркам: {pct(ns['ES_95'])} против {pct(ew['ES_95'])} у Equal Weight и {pct(spy['ES_95'])} у SPY.
...
Short-allowed стратегия оказалась слабее базовой no-short версии: Sharpe {short['sharpe_ratio']:.3f} против {ns['sharpe_ratio']:.3f}, cumulative return {pct(short['cumulative_return'])} против {pct(ns['cumulative_return'])}. Это показывает, что разрешение коротких продаж в данной выборке не улучшило итоговое качество и увеличило чувствительность к ошибкам оценки $\\mu$ и $\\Sigma$. Поэтому основной результат проекта — именно no-short оптимальный портфель.
